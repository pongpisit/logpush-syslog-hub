...
|---|---|
| Publicly reachable, **no IP-based firewall/security group** | `Direct` — plain TCP straight to the host. |
...
| **Test send** reports `{"ok":true}` (or the Dashboard shows a successful delivery), but **nothing ever arrives** at the destination | Check what your destination's `host` actually resolves to: `dig +short <host>`. If it comes back with a Cloudflare anycast IP (commonly in the `104.16.0.0/12`–`104.31.255.255` range, among others), that hostname is **proxied through Cloudflare** (orange cloud) rather than pointing at your real origin — and [outbound TCP sockets to Cloudflare IP ranges are blocked](https://developers.cloudflare.com/workers/runtime-apis/tcp-sockets/#considerations). The connection attempt can still resolve without throwing an error from this Worker's point of view, which is why it's reported as a success even though the write never reaches your actual server. Fix: switch that DNS record to **DNS only** (grey cloud) so it resolves to your real origin, or configure the destination with the raw origin IP directly instead of a proxied hostname. |
